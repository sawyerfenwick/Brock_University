 ...
 *See also: BinaryNode, BinaryTreeIteratorBFS
 *Brock University
 */
public class BinarySearchTree<E extends Comparable<E>> implements Iterable<E> {
	...
	public void add(E toInsert) {
		BinaryNode<E> ptr;
		if (myTree==null) {
			myTree=new BinaryNode<>(toInsert);
		}
		else {
			ptr=myTree;
			while (true) {
				if (toInsert.compareTo(ptr.info)>0) { //Right branch
					if (ptr.right==null) { //Stick it here
						ptr.right=new BinaryNode<>(toInsert);
						break;
					}
					else ptr=ptr.right;
				}
					...
					if (ptr.left==null) { //Stick it here
						ptr.left=new BinaryNode<>(toInsert);
						break;
					}
					else ptr=ptr.left;
				}
	...
	BinaryNode<E> findMax(BinaryNode<E> node) {
		if (node!=null) while (node.right!=null) node=node.right;
		return node;
	}
	
	//From the book
	BinaryNode<E> remove(E val, BinaryNode<E> tree) {
	...
	public Iterator<E> iterator() {
		return new BinaryTreeIteratorBFS<>(myTree);
	}
}
